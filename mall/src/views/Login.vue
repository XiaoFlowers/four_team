<script setup>
	...
	const tologin = () => {
		...
		ajax.open('get', 'http://localhost:3000/login');
		ajax.send();
		ajax.onreadystatechange = function() {
			if (ajax.readyState == 4 && ajax.status == 200) {
				...
				for(let i=0;i<arr.length;i++){
					// console.log(arr[i].username,store.user);
					if(arr[i].usename==store.user&&arr[i].password==store.pass)
					{
						router.push('/about')
					}
					else{
...
<template>
	<video width="300" height="300" loop playsinline autoplay src="../assets/1.mp4" tabindex="-1" muted="muted"></video>
	<div class="shell">
		...
		<div class="container b-container" id="b-container">
			<form action="" method="" class="form" id="b-form">
				...
				<input type="text" class="form_input" placeholder="Name" @input="user">
				<input type="text" class="form_input" placeholder="Password" @input="pass">
				<a class="form_link">忘记密码？</a>
				<button class="form_button button submit" @click="tologin">SIGN IN</button>
			</form>
		</div>
